...
const EMERGENCY_NUMBERS: Record<string, EmergencyNumbers> = {
  FR: { country: 'France',       police: '17',  fire: '18',  ambulance: '15',  general: '112' },
  MA: { country: 'Maroc',        police: '19',  fire: '15',  ambulance: '15',  general: '15'  },
  TN: { country: 'Tunisie',      police: '197', fire: '198', ambulance: '190'               },
  DZ: { country: 'Algérie',      police: '17',  fire: '14',  ambulance: '14',  general: '1021'},
  SN: { country: 'Sénégal',      police: '17',  fire: '18',  ambulance: '15'                },
  CM: { country: 'Cameroun',     police: '17',  fire: '18',  ambulance: '15'                },
  CI: { country: "Côte d'Ivoire",police: '111', fire: '180', ambulance: '185'               },
  US: { country: 'États-Unis',   police: '911', fire: '911', ambulance: '911', general: '911'},
  GB: { country: 'Royaume-Uni',  police: '999', fire: '999', ambulance: '999', general: '112'},
  DE: { country: 'Allemagne',    police: '110', fire: '112', ambulance: '112'               },
  ES: { country: 'Espagne',      police: '091', fire: '080', ambulance: '061', general: '112'},
  IT: { country: 'Italie',       police: '113', fire: '115', ambulance: '118', general: '112'},
  ...
  DEFAULT: { country: 'International', police: '112', fire: '112', ambulance: '112', general: '112' },
};
...
export class SosService {
  ...
  async getContacts(userId: string) {
    return this.contactModel.find({ userId: new Types.ObjectId(userId) }).lean();
  }

  async createContact(userId: string, dto: CreateEmergencyContactDto) {
    const contact = new this.contactModel({ userId: new Types.ObjectId(userId), ...dto });
    await contact.save();
    return contact;
  }

  async updateContact(contactId: string, userId: string, dto: Partial<CreateEmergencyContactDto>) {
    const contact = await this.contactModel.findById(contactId);
    if (!contact) throw new NotFoundException('Contact introuvable');
    if (!contact.userId.equals(new Types.ObjectId(userId))) throw new ForbiddenException('Accès interdit');

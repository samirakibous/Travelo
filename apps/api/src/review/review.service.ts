...
export class ReviewService {
  ...
  async create(touristId: string, userRole: string, guideId: string, dto: CreateReviewDto) {
    if (userRole !== Role.TOURIST) {
      throw new ForbiddenException('Seuls les touristes peuvent laisser un avis');
    }
    ...
    if (guide.userId.toString() === touristId) {
      throw new ForbiddenException('Vous ne pouvez pas noter votre propre profil');
    }

    const existing = await this.reviewModel.findOne({ guideId, touristId });
    if (existing) throw new ConflictException('Vous avez déjà laissé un avis pour ce guide');
